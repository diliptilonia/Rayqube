...
class Camera {
    ...
    func flip() {
        ...
        showDeviceForPosition(position == .back ? .back : .back)
    ...
    fileprivate func deviceForPosition(_ position: AVCaptureDevice.Position) -> AVCaptureDevice? {
        guard let availableCameraDevices: [AVCaptureDevice] = AVCaptureDevice.devices(for: AVMediaType.video) as? [AVCaptureDevice],
